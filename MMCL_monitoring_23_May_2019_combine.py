...
from tkinter import * #need for .pack(side=TOP)
...
class idd_class:
    ...
    #try:
        ...
        # save file
        date_file = (ws_idd["C5"].value).split("-")
        print(date_file)
...
class upload:
    ...
    def summary(self, all_dic):
        ...
        print("loop checker value = ",upload.check_loop)
        print("all city trunk list : ",self.all_city_trunk)
        return None
    ...
    def summary_ios(self,all_dic):
        ...
        # ******************************************************************************************
        print(all_dic)
        print("all_dic = ",all_dic[0])
        print("ios_trunk_list = ",ios_trunk_list)
        print("ios trunk list[0] = ",ios_trunk_list[0])
...
class open_cl:
    ...
    def maximize(cls):
        # global = open_close = 2
        if open_cl.open_close%2 == 0:
            top.geometry("%dx%d+%d+%d" % (width_of_window, height_of_window+300, x_coordinate, y_coordinate-150));
            open_cl.open_close += 1
        else:
...
class Tools:
    ...
    def d_clock(cls):
        ...
        def clock_refresher():
            ...
            clock.after(100, clock_refresher)
        clock_refresher()
        return None
...
class Mail:
    ...
    def kpi(cls):
        to="raj@mmclbd"
        cc="sharma@mmclbd;anirban@mmclbd;arif@mmclbd;noc@mmclbd"
        subject = f"ICX KPI report on June 2019"
        try:
            attach = upload.nam_temp_kpi
        except Exception as e:
            print("Error type in KPI : ",e)
            attach = None

        body = r'<body style="font-family:Verdana;font-size:13.5" > Dear Vaiya, <br> ' \
               r'Please check the domestic kpi report on June 2019. <br><br>' \
               r'Regards <br>' \
               r'HR {to} <br>' \
               r'<p> <img src="‪‪‪C:\Users\Habib\Pictures\advance redistribution pic.png"></p> <br>' \
               r'<p> < img src = "C:\Users\Habib\Pictures\Bank Job Application.png" > </p>' \
               r'01777722222<br>' \
               r'noc@mmclbd.com' \
               r'</body>'

        body = body + r'<body><p><img src="C:\Users\Habib\Pictures\Bank Job Application.png"></p></body>'

        cls.creat_mail(to=to,cc=cc,subject=subject,body=body,attach=attach)
        return None
    ...
    def ios_idd(cls):
        to = "raj@mmclbd"
        cc = "sharma@mmclbd;anirban@mmclbd;arif@mmclbd;noc@mmclbd"
        subject = f"IOS IDD report on June 2019"
        try:
            attach = upload.nam_temp_ios
        except Exception as e:
            print("Error type in IOS ISD : ",e)
            attach=None
        body=""
    ...
    def idd(cls):
        to="raj@mmclbd"
        cc="sharma@mmclbd;anirban@mmclbd;arif@mmclbd;noc@mmclbd"
        subject = f"IDD report on June 2019"
        attach = upload.nam_temp_idd
        body=r'<body> Dear Vaiya, <br> ' \
             r' Please check the IDD report on June 2019. <br><br>' \
             r'Regards <br>' \
             r'HR <br>' \
             r'<p> <img src = "‪‪C:\ramdisk speed test.PNG"><br>' \
             r'01777722222<br>' \
             r'noc@mmclbd.com' \
             r'</p></body>'
    ...
    @classmethod
    def idd_every_two_hours(cls):
        to="raj@mmclbd"
        cc="anirban@mmclbd;arif@mmclbd;noc@mmclbd"
        subject = f"IDD report on June 2019 Every Two hours"
        try:
            attach = idd_class.filename_mail
        except Exception as e:
            print("Error type in idd every 2 hours : ",e)
            attach = None
        body=r'<body> Dear Vaiya, <br> ' \
             r' Please check the IDD report on June 2019 Every two hours. <br><br>' \
             r'Regards <br>' \
             f'HR <br>' \
             r'<p> <img src = "‪‪C:\ramdisk speed test.PNG"></p><br>' \
             r'01777722222<br>' \
             r'noc@mmclbd.com' \
             r'</body>'
        # body = body + r""
...
if __name__ == "__main__":
    ...
    open_cl.open_close = 2
    '''
    Starting point of ccr isd report
    '''
    ...
    if True:    # this if used only for minimize code
            # Make menu bar...
        menubar = Menu(top)
        file = Menu(menubar)
        file.add_command(label="Set Header Color",command=open_cl.HeaderColor)
        file.add_command(label="Set Side color",command=open_cl.sideColor)
        file.add_separator()
        ...
        mail_menu = Menu(menubar)
        mail_menu.add_command(label = "create new Mail",command = Mail.creat_mail)
        mail_menu.add_command(label = "IDD Every Two Hours",command = Mail.idd_every_two_hours)
        mail_menu.add_command(label = "KPI report",command = Mail.kpi)
        mail_menu.add_command(label = "IOS ISD report",command = Mail.ios_idd)
        mail_menu.add_command(label = "IDD report",command = Mail.idd)
    ...
    #var_2 = tkinter.StringVar(); var_2.set(ins_text_1.filename);
    ...
    # btn_2 = tkinter.Button(frm_1, textvariable=var_2, command=ins_text_1.csv_file, width=width_1, font=font,bg=bg_1,fg=fg_1);
    ...
    var_3 = StringVar(); var_3.set("Current directory \n" + getcwd());
    lab_1 = Label(frm, textvariable=var_3, width=width_1, height=2, font=font)
    lab_1.grid(row=4,column=0)

    frm.grid(row=2,column = 0)      # this is the main frame for ccr idd
        # set a button for extend window size to show KPI report part
    Button(frm,text="KPI report",font = font,width=width_1,command = open_cl.maximize).grid(row=5,column = 0)
    ...
    bg_kpi1 = "#00ff00"
    bg_kpi2 = "#99aa00"
    Label(top).grid(row=1,column = 0)   # gap between two part

    raw_kpi_in_instance  = upload();
    raw_kpi_out_instance = upload();
    raw_idd     = upload();

    frm_kpi = Frame(top,bd=5,relief="solid",pady=5,padx=5,bg="#ffffff")
    ...
    raw_in_kpi_file_path = StringVar(); raw_in_kpi_file_path.set("Open incoming KPI .csv")
    raw_out_kpi_file_path= StringVar(); raw_out_kpi_file_path.set("Open outgoing KPI .csv")
    raw_idd_file_path    = StringVar(); raw_idd_file_path.set("Open IDD .csv")

    btn_1 = Button(raw_frame, command=raw_kpi_in_instance.upload_kpi, textvariable=raw_in_kpi_file_path, width=width_1, font="Times 15", bg=bg_kpi1, anchor="w");
    btn_2 = Button(raw_frame, command=raw_kpi_out_instance.upload_kpi_out, textvariable=raw_out_kpi_file_path, width=width_1, font="Times 15",bg=bg_kpi1, anchor="w");
    btn_3 = Button(raw_frame,command=raw_idd.upload_idd,textvariable=raw_idd_file_path,width=width_1,font="Times 15", bg=bg_kpi1, anchor="w");
    ...
    btn_kpi = Button(make_frame,text="Make KPI Report",command=raw_kpi_in_instance.make_kpi_report,width=width_2, font=font, bg=bg_kpi2)
    btn_kpi.grid(row=0,column=0);

    btn_ios = Button(make_frame,text="Make IOS IDD Report",command=raw_kpi_out_instance.make_IOS_ISD_report,width=width_2, font=font, bg=bg_kpi2)
    btn_ios.grid(row=0,column=1);

    btn_idd = Button(make_frame,text="Make IDD Report",command=raw_idd.make_IDD_report,width=width_2, font=font, bg=bg_kpi2)
    ...
    kip_conform.set(" ")
    Label(frm_kpi, textvariable = kip_conform, font=font, width=width_1).grid(row=5, column = 0)
