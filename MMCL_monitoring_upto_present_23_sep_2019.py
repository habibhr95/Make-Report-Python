'''
Required packages
pip install openpyxl   For Excel control
pip install pywin32    For Mailing
...
class upload:
    ...
    def upload_kpi(self):
        filename = filedialog.askopenfilename(title = "Select incoming KPI raw CSV file",filetypes = (("csv files","*.csv"),("all files","*.*")))
        if filename != '':
            self.filename = filename
    ...
    def upload_kpi_out(self):
        filename = filedialog.askopenfilename(title = "Select outgoing KPI Raw CSV File",filetypes = (("csv files","*.csv"),("all files","*.*")))
        if filename != '':
            self.filename = filename
    ...
    def upload_idd(self):
        filename = filedialog.askopenfilename(title = "Select idd Raw CSV File",filetypes = (("csv files","*.csv"),("all files","*.*")))
        if filename != '':
            self.filename = filename
    ...
    def make_IOS_ISD_report(self):
        ...
        if raw_kpi_in_instance.ws_raw_data['A1'].value == None:     # to make IOS ISD report must make KPI report first.
            ...
            return None
        print('value of A1 raw kpi ',raw_kpi_in_instance.ws_raw_data['A1'].value)
    ...
    def Help_doc_login(cls):
        ...
        def login():
            ...
            if not os.path.exists(r'data/password/password.txt'):
                h = hashlib.md5("1".encode())
                h=h.hexdigest()
                file_1 = open(r'data/password/password.txt', "w+")
                file_1.write(str(h))
                print("password file created ",h)
...
class extraInfo:
    ...
    def show_image_doc():
        ...
        dirlist = all_image_names
        old_label_image = None
        for f in dirlist:
            try:
                image1 = Image.open(f"{path}/{f}")
