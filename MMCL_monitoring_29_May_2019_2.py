...
# if os.path.isdir(out_folder):pass
# else:Options.change_out_folder()
...
class idd_class:
    ...
    def csv_file(self):
        ...
        upload.set_border(None,ws=ws_idd,cell_range=cell_range)

        top_cell_range = [1,2,1,ws_idd.max_column]  # set style top row/ Header row
        upload.set_border(None,ws=ws_idd,cell_range=top_cell_range,bd_sty="medium",font_weight=True,bg_color=header_color)  # Border width = medium valid !!

        top_cell_range = [2,ws_idd.max_column-2,ws_idd.max_row,ws_idd.max_column]  # set style ASR,ACD,CCR
        upload.set_border(None,ws=ws_idd,cell_range=top_cell_range,bd_sty="thin",font_weight=True,bg_color=side_color)
        ...
        except Exception as e: print("output folder not set ",e)
...
class ccr_class:
    ...
    def csv_file(self):
        ...
        if True:
            ...
            upload.set_border(None,ws=ccr,cell_range=[1,1,1,ccr.max_column],bd_sty="medium",font_weight=False,bg_color=header_color)
            # set border to data cells
            upload.set_border(None,ws=ccr,cell_range=[2,1,ccr.max_row,ccr.max_column])
...
class upload:
    ...
    def make_IOS_ISD_report(self):
        ...
        if int(self.total_time) == int(sum2):
...
class Open_gui:
    ...
    def kpi_gui(cls):
        ...
        file.add_command(label="Set Header Color", command=Options.headerColor)
        file.add_command(label="Set Side color", command=Options.sideColor)
        file.add_command(label="Current Directory", command=Options.current_d)
        file.add_separator()
        ...
        # Exit button
        Button(kpi_top,text='Exit',font=font,command=des,relief="raised",bd=10).grid()
        return None
...
class Options:   # Menu make
    ...
    def current_d(cls):
        ...
        def auto_mail_opt():
            nonlocal auto_mail
            if auto_mail_enable.get() is 0:
                auto_mail = "Disabled"
            elif auto_mail_enable.get() is 1:
                auto_mail = "Enabled"

        auto_mail_opt()

        top = Toplevel()
        top.title("Save Directory")
        top.resizable(0, 0)

        fram_top = Frame(top)
        frm = Frame(fram_top, bg="white", bd=5)
        Label(frm, text="Current Directory: ", font=font, bg="white").grid(row=2, column=0)
        Label(frm, text=f' {os.getcwd()} ', font=font, bg="white", bd=5).grid(row=2, column=1)

        Label(frm, text="Output Folder: ", font=font, bg="white").grid(row=3, column=0)
        Label(frm, text=f' {out_folder_dir} ', font=font, bg="white", bd=5).grid(row=3, column=1)
        frm.grid(row=0, column=0)
        #################################

        frm2 = Frame(fram_top, bg="white", bd=5)
        Button(frm2, text="Header color", command=Options.headerColor, bg=f"#{header_color}", font=font).grid(row=0,
                                                                                                              column=0)
        Button(frm2, text="Side color", command=Options.sideColor, bg=f"#{side_color}", font=font).grid(row=0, column=1)
        frm2.grid(row=1, column=0)

        Label(frm2, text=f"Employee : ", font=font).grid(row=1, column=0)
        Label(frm2, text=f"{mail_instance.name}", font=font).grid(row=1, column=1)

        Label(frm2, text=f"Auto Mailing : ", font=font).grid(row=2, column=0)
        Label(frm2, text=f"{auto_mail}", font=font).grid(row=2, column=1)

        # btn1 = Button(top,text="Exit",font=font,bg="white",command=top.destroy)
        btn1 = Button(fram_top, text="Exit", font=font, command=partial(lambda: top.destroy()))
...
class Mail:
    ...
    def kpi(self):
        ...
        try:
            attach = os.path.normpath(out_folder_dir + os.sep + upload.nam_temp_kpi)
        except Exception as e:
    ...
    def ios_idd(self):
        ...
        try:
            attach = os.path.normpath(out_folder_dir + os.sep + upload.nam_temp_ios)
        except Exception as e:
            print("Error type in KPI : ", e)
            attach = None
    ...
    def idd(self):
        ...
        # set attached file name.
        try:
            attach = os.path.normpath(out_folder_dir + os.path.sep + upload.nam_temp_idd)
        except Exception as e:
            print("Error type in KPI : ", e)
            attach = None
...
if __name__ == "__main__":
    ...
    btn_1 = Button(frm_1, textvariable=var_1, command=ins_text_1.upload, width=width_1, font=font,bg=bg_1,fg=fg_1,
                   relief=relief,bd=bd)
    btn_3 = Button(frm_1, textvariable=var_3, command=ins_text_1.csv_file, width=width_1, font=font,bg=bg_1,
                   relief=relief,bd=bd,fg=fg_1);
    ...
    btn1 = Button(frm_2,textvariable=var1,command=ins_text.upload,width=width_1,font=font,
                  relief=relief,bd=bd,bg=bg_2,fg=fg_1);
    btn2 = Button(frm_2,textvariable=var2,command=ins_text.csv_file,width=width_1,font=font,
                  relief=relief,bd=bd,bg=bg_2, fg=fg_1);
    ...
    lab_1 = Label(frm, font=font,bg="white",width=width_1,text=f'{os.getcwd()}')
