...
class idd_class:
    ...
    def csv_file(self):
        ...
        if len(fd1.readline().split('","')) != 43:
            ...
            messagebox.showwarning("warning", "Wrong in put file.")
            return None
...
class upload:
    ...
    def uploaded_cvs_to_xlsx_kpi(self):
        ...
        try:
            ...
            print("Please in put file")
            text_var_out_folder.set("Please in put first.")
            messagebox.showwarning("warning", "Invalid Input\nPlease in put first.", parent=kpi_top)
            return None

        if len(fd1.readline().split('","')) != 45:
            text_var_out_folder.set("Wrong in put file.")
            messagebox.showwarning("warning", "Wrong in put file.", parent=kpi_top)
            return None
    ...
    def make_IOS_ISD_report(self):
        ...
        if int(self.total_time) == int(sum2):
            ...
            messagebox.showerror("error", "Error \n Wrong Input file",parent=kpi_top)
            dont_mail=0
...
class Open_gui:
    ...
    def kpi_gui(cls):
        ...
        # Exit button
        Button(frm_kpi,text='Exit',font=font,command=des,width=width_1,relief="raised",
    ...
    def spc_converter(cls):
        ...
        def calculate():
            ...
            try:
                ...
                messagebox.showwarning("warning", "Invalid Input\nEnter 3-8-3 Format 1", parent=top_spc)
                return None
            c=[2048,8,1]
            dec_spc=0
            for itm in range(3):
...
class Options:   # Menu make
    ...
    def current_d(cls):
        ...
        def header_func():
            Options.headerColor()
            Button(frm3, text="Header color", width=abc2, command=header_func, bg=f"#{header_color}", font=font,
                   bd=5).grid(row=4, column=0)
        def side_func():
            Options.sideColor()
            Button(frm3, text="Side color", width=abc2, command=side_func, bg=f"#{side_color}", font=font,
                   bd=5).grid(row=4, column=1)
        frm3 = Frame(fram_top)
        Button(frm3, text="Header color",width=abc2, command=header_func, bg=f"#{header_color}", font=font,
               bd=5).grid(row=4, column=0)
        Button(frm3, text="Side color",width=abc2, command=side_func, bg=f"#{side_color}", font=font, bd=5
               ).grid(row=4, column=1)
        frm3.grid()
...
def setting_var():
    ...
    global text_var_emp_name, text_var_auto_mail_e, text_var_out_folder
...
if __name__ == "__main__":
    header_color = "ff0000" # header color of excel file
    side_color = "00ff00"   # side/result color of excel file
    ...
    btn_1 = Button(frm_1, textvariable=var_1, command=ins_text_1.upload, width=width_1, font=font,
                   bg=bg_1,fg=fg_1,relief=relief,bd=bd)
    btn_3 = Button(frm_1, textvariable=var_3, command=ins_text_1.csv_file, width=width_1, font=font,
    ...
    Label(frm_check_b,text=" CCR Check ",bg=bg_21,width=width_1//2+20,font=font,height=2, fg=fg_1).grid(row=0,column=0)
    intVar_ccr_auto_open = IntVar();
    Checkbutton(frm_check_b,text="Auto Open Excel",bg=bg_21,width=width_1//2-22,font=font,height=1, fg=fg_1,onvalue=1,
                offvalue=0,variable=intVar_ccr_auto_open).grid(row=0,column=1)
